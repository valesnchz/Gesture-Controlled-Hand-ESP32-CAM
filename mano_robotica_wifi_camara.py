...
=============================================================
 ...
 2. Procesa la IA (MediaPipe) en la PC con pura matemática (sin TFLite).
 3. Manda las órdenes a los servos por WiFi de forma ultra-rápida sin lag.
=============================================================
"""
...
# ─────────────────────────────────────────────────────────
#  1. CONFIGURACIÓN WIFI (ESP32-CAM)
# ─────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────
#  2. CONFIGURACIÓN DEL MODELO DE MEDIAPIPE
# ─────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────
TIP_IDS = [4, 8, 12, 16, 20] # Puntas de los dedos (Pulgar, Índice, Medio, Anular, Meñique)
...
# ─────────────────────────────────────────────────────────
#  4. CONEXIÓN A LA CÁMARA WIFI DE LA ESP32-CAM
# ─────────────────────────────────────────────────────────
print(f"[INFO] Conectando al video stream en {URL_VIDEO}...")
cap = cv2.VideoCapture(URL_VIDEO)

if not cap.isOpened():
    print("[ERROR] No se pudo abrir la transmisión de video de la ESP32-CAM.")
    print("        Asegúrate de estar conectado a la red WiFi 'CamaraESP32'.")
    sys.exit(1)

cv2.namedWindow("Mano Robotica - ESP32-CAM", cv2.WINDOW_NORMAL)
cv2.resizeWindow("Mano Robotica - ESP32-CAM", 640, 480)

# ── HISTÉRESIS PARA DEDOS MÁS ESTABLES (EVITA TEMBLORES) ───
...
last_send_time = 0
send_interval = 0.05  # 50 ms entre envíos (ahora mucho más rápido gracias al hilo de fondo)
timestamp_ms = 0
...
while True:
    ...
    # Espejo e información del frame
    ...
    if seguimiento_activo:
        ...
        if ultimo_resultado and ultimo_resultado.hand_landmarks and len(ultimo_resultado.hand_landmarks) > 0:
            ...
            for lm in ultimo_resultado.hand_landmarks:
                ...
                # --- PURE MATH: CÁLCULO DE DEDOS ABIERTOS/CERRADOS ---
                fingers = []
                wrist = lm[0]

                # ── 1. Dedo Pulgar (Inclinación lateral vs base meñique) ──
                ...
                # ── 2. Los otros 4 dedos (Índice, Medio, Anular, Meñique) ──
                for i in range(1, 5):
